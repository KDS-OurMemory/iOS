//
//  FriendsCollectionAdapter.swift
//  OurMemory
//
...
enum FRIENDSCOLLECTION_SECTION:Int {
    case FRIENDSSEARCH_SECTION = 0
    case FRIENDSLIST_SECTION = 1
}

class FriendsCollectionAdapter: BaseCollectionAdapter {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case FRIENDSCOLLECTION_SECTION.FRIENDSLIST_SECTION.rawValue:
            if let data = self.getIndexPathDataAt(indexPath: indexPath) as? FriendsDataBinder {
                if let friendCell = collectionView.dequeueReusableCell(withReuseIdentifier: OurMemoryFriendsCollectionViewCell.className(), for: indexPath) as? OurMemoryFriendsCollectionViewCell {
